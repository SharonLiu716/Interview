...
sim.BAC2<-function(seq,dataType){
    ...
    while(TRUE){
      ...
      CI<-tryCatch({
        ...
        S.na <- function(e){
          ...
          s02 = seq * ( mean(yy$y13) - exp(mlee[1]+e+mlee[5])+ mean(yy$y22) - exp(mlee[1]+e+mlee[4]+mlee[6]) + mean(yy$y32)- exp(mlee[1]+e+mlee[4]+mlee[7]) )
...
res21<-do.call(rbind, lapply(c(25,50,150,250), sim.BAC2, dataType="Poisson")) 
write_xlsx(data.frame(res21),path = paste0(Path,"/BAC2_funPo_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))

res22<-do.call(rbind, lapply(c(25,50,150,250), sim.BAC2, dataType="Weibull")) 
write_xlsx(data.frame(res22),path = paste0(Path,"/BAC2_funWei_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))
