...
MatAB.Po.BBAACBCAC<-function(DT,NN,YY,MLE,Xs,mat.type){
  ...
  vv.te1 =  (yii[1]+yii[2]+yii[6])/3+(2*yij[1]+yij[2]+yij[3]+yij[5]+yij[6])/3
  vv.te2 =  (yii[5]+yii[7]+yii[9])/3+(yij[4]+yij[6]+yij[7]+2*yij[8]+yij[9])/3
  ...
  vv.e1e1 =  (yii[1]+yii[2]+yii[6]+2*yij[1])/3
  vv.e1e2 =  yij[6]/3
  vv.e1g1 = (yii[2]+yij[1]+yij[6])/3
  vv.e1g2 = (yii[6]+yij[2]+yij[3])/3
  vv.e1d1 = (yii[6]+yij[5]+yij[6])/3
  vv.e1d2 = vv.d1d2 = 0
  
  vv.e2e2 =  (yii[5]+yii[7]+yii[9]+2*yij[8])/3
  vv.e2g1 = (yii[5]+yij[7]+yij[9])/3
  vv.e2g2 = (yii[9]+yij[6]+yij[8])/3
  vv.e2d1 = (yii[5]+yij[4]+yij[6])/3
  vv.e2d2 = (yii[7]+yii[9]+yij[7]+2*yij[8]+yij[9])/3
...
#============================================
Path="C:/0409pp"
# Path="C:/Users/User/Documents/Study_CrossoverDesign/BBAACBCAC"
setwd(Path)

sim_time=10000;cor_par=0.4;Type='BBAACBCAC'
...
sim.BBA2<-function(seq,dataType){
    ...
    while(TRUE){
      ...
      CI<-tryCatch({
        if (dataType == "Poisson"){
          ...
          yy<-Data.Wei.BBAACBCAC(Rho=cor_par,NN=seq,W.shape=W.aa,W.scale=W.bb,W.time=W.t)
          Y<-as.vector(unlist(yy))
        }
        X1 = c(rep(1,each=seq,time=2),rep(0,each=seq,time=3),rep(1,seq),rep(0,each=seq,time=3))
        X2 = c(rep(0,each=seq,time=3),rep(0:1,each=seq,time=3))
        ...
        mle.0[i,]<- append(mod.0$coefficients, 0, after=2) # eta1 =0 after 1 ,eta2 = 0 after 2
        #---------------------------------
        #Matrix I & Matrix V
        #---------------------------------
        Mat = MatAB.Po.BBAACBCAC(DT=Type,NN=seq,YY=yy,MLE=mle.1[i,],Xs = xmat,mat.type = 'true')
        A[[i]]<-Mat$Mat.A;    B[[i]]<-Mat$Mat.B
        
        var.na[i]<-(solve(A[[i]])/(3*seq))[2,2] # eta1 = [1,1] ,eta2 = [2,2]
        var.rb[i]<-(solve(A[[i]]) %*% B[[i]] %*% solve(A[[i]])/(3*seq))[2,2] # eta1 = [1,1] ,eta2 = [2,2]
        #---------------------------------
        #log LR test 
        #---------------------------------
        lik<-function(par){ 
          ll=sum((par[1]+par[2])*yy$y11-exp(par[1]+par[2])+(par[1]+par[2]+par[4])*yy$y12-exp(par[1]+par[2]+par[4])+(par[1]+par[5])*yy$y13-exp(par[1]+par[5]))+
            ...
            sum((par[1]+par[3]+par[7])*yy$y31-exp(par[1]+par[3]+par[7])+ (par[1]+par[4]+par[7])*yy$y32-exp(par[1]+par[4]+par[7]) + (par[1]+par[3]+par[5]+par[7])*yy$y33-exp(par[1]+par[3]+par[5]+par[7]))
          return(ll)
        }
        #=====================================
        #LR Test
        ...
        Lna1[i]<-2*(l1-l0)
        Lrb1[i]<-2 * (A[[i]] %*% solve(B[[i]]))[2,2] * (l1-l0)# eta1 = [1,1] ,eta2 = [2,2]
        ...
        LR.naci<-findUL(step=0.01, fun=LR.na, MLE = mle.1[i,3]) # eta1 = mle.1[i,2]  ,eta2 = mle.1[i,3]
        ...
        LR.rb <- function(e){
          ...
          mlee<- append(Mod.0$coefficients, e, after=2) # eta1 =0 after 1 ,eta2 = 0 after 2
          l.0 = lik(mlee)
          u = 2* (A[[i]] %*% solve(B[[i]]))[2,2]*(l1-l.0)
          list("-2LLR"=u)
        }
        
        LR.rbci<-findUL(step=0.01, fun=LR.rb, MLE = mle.1[i,3]) # eta1 = mle.1[i,2]  ,eta2 = mle.1[i,3]
        ...
        #---------------------------------
        Mat0 = MatAB.Po.BBAACBCAC(DT=Type,NN=seq,YY=yy,MLE=mle.0[i,],Xs = xmat,mat.type = 'null')
        ...
        # = c(mle.0[i,1],0,mle.0[i,3],mle.0[i,4],mle.0[i,5],mle.0[i,6],mle.0[i,7])
        # s0.1 = seq * ( mean(yy$y12) - exp(mle.0[i,1]+mle.0[i,4])+ mean(yy$y21) - exp(mle.0[i,1]+mle.0[i,6]) + mean(yy$y33)- exp(mle.0[i,1]+mle.0[i,5]+mle.0[i,7]) )
        s0.2 = seq * ( mean(yy$y22) - exp(mle.0[i,1]+mle.0[i,4]+mle.0[i,6])+ mean(yy$y31) - exp(mle.0[i,1]+mle.0[i,7]) + mean(yy$y33)- exp(mle.0[i,1]+mle.0[i,5]+mle.0[i,7]) )
        Sna1[i]<- s0.2 %*% solve(A0[[i]])[2,2] %*% t(s0.2) / (3*seq) # eta1 = [1,1] ,eta2 = [2,2]
        Srb1[i]<- s0.2%*% solve(B0[[i]])[2,2] %*% t(s0.2) / (3*seq) # eta1 = [1,1] ,eta2 = [2,2]
        ...
        S.na <- function(e){
          ...
          Mod.0 <- glm(formula = Y~ X1 + Z1 + Z2 + G1 + G2+offset(e*X2),family = poisson(link = "log"), data = df.cor)
          mlee<- append(Mod.0$coefficients, e, after=2) # eta1 =0 after 1 ,eta2 = 0 after 2
          mat = MatAB.Po.BBAACBCAC(DT=Type,NN=seq,YY=yy,MLE=mlee,Xs = xmat,mat.type = 'null')
          aa<-mat$Mat.A;    bb<-mat$Mat.B
          s02 = seq * ( mean(yy$y22) - exp(mlee[1]+e+mlee[4]+mlee[6])+ mean(yy$y31) - exp(mlee[1]+e+mlee[7]) + mean(yy$y33)- exp(mlee[1]+e+mlee[5]+mlee[7]) )
          s.na = s02^2 * solve(aa)[2,2]  / (3*seq) # eta1 = [1,1] ,eta2 = [2,2]
          list("-2LLR"=s.na)
        }
        
        S.naci<-findUL(step=0.01, fun=S.na, MLE = mle.1[i,3]) # eta1 = mle.1[i,2]  ,eta2 = mle.1[i,3]
        ...
        S.rb <- function(e){
          ...
          aa<-mat$Mat.A;    bb<-mat$Mat.B
          s02 = seq * ( mean(yy$y22) - exp(mlee[1]+e+mlee[4]+mlee[6])+ mean(yy$y31) - exp(mlee[1]+e+mlee[7]) + mean(yy$y33)- exp(mlee[1]+e+mlee[5]+mlee[7]) )
          s.rb = s02^2 * solve(bb)[2,2]  / (3*seq) # eta1 = [1,1] ,eta2 = [2,2]
          list("-2LLR"=s.rb)
        }
        
        S.rbci<-findUL(step=0.01, fun=S.rb, MLE = mle.1[i,3]) # eta1 = mle.1[i,2]  ,eta2 = mle.1[i,3]
        ...
        #wald test
        #=====================================
        Wna1[i]<-mle.1[i,3]^2 / ( solve(A0[[i]]) / (3*seq) )[2,2] # eta1 = [1,1] ,eta2 = [2,2]
        Wrb1[i]<- mle.1[i,3]^2 / (solve(A0[[i]]) %*% B0[[i]] %*% solve(A0[[i]])/(3*seq))[2,2]
        #=====================================
        ...
        W.na <- function(e){
          ...
          mat = MatAB.Po.BBAACBCAC(DT=Type,NN=seq,YY=yy,MLE=mlee,Xs = xmat,mat.type = 'null')
          aa<-mat$Mat.A;    bb<-mat$Mat.B
          w.na = (mle.1[i,3]-e)^2 / ( solve(aa) / (3*seq) )[2,2] # eta1 = [1,1] ,eta2 = [2,2]
          list("-2LLR"=w.na)
        }
        W.naci<-findUL(step=0.01, fun=W.na, MLE = mle.1[i,3])
        ...
        W.rb <- function(e){
          Mod.0 <- glm(formula = Y~ X1 + Z1 + Z2 + G1 + G2+offset(e*X2),family = poisson(link = "log"), data = df.cor)
          mlee<- append(Mod.0$coefficients, e, after=2)
          mat = MatAB.Po.BBAACBCAC(DT=Type,NN=seq,YY=yy,MLE=mlee,Xs = xmat,mat.type = 'null')
          aa<-mat$Mat.A;    bb<-mat$Mat.B
          w.rb = (mle.1[i,3]-e)^2 / (solve(aa) %*% bb %*% solve(aa)/(3*seq))[2,2] 
          list("-2LLR"=w.rb)
        }
        
        W.rbci<-findUL(step=0.01, fun=W.rb, MLE = mle.1[i,3])
  ...
  VARs<-c(var(mle.1[,3]),sd(mle.1[,3]),mean(var.na),sqrt(mean(var.na)),mean(var.rb),sqrt(mean(var.rb)))
  names(VARs)<-c("sv.eta2","std.eta2","var.na.eta2","std.na.eta2","var.rb.eta2","std.rb.eta2")
...
# test22<-sim.BBA2(150,"Weibull")

res21<-do.call(rbind, lapply(c(25,50,150,250), sim.BBA2, dataType="Poisson")) 
write_xlsx(data.frame(res21),path = paste0(Path,"/BBA2_funPo_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))

res22<-do.call(rbind, lapply(c(25,50,150,250), sim.BBA2, dataType="Weibull")) 
write_xlsx(data.frame(res22),path = paste0(Path,"/BBA2_funWei_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))
