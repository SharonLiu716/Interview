...
sim.BBA1<-function(seq,dataType){
  ...
  for (i in 1:sim_time){
    while(TRUE){
      ...
      CI<-tryCatch({
        ...
        s0.1 = seq * ( mean(yy$y11) - exp(mle.0[i,1])+ mean(yy$y12) - exp(mle.0[i,1]+mle.0[i,4]) + mean(yy$y23)- exp(mle.0[i,1]+mle.0[i,5]+mle.0[i,6]) )
        ...
        S.na <- function(e){
          ...
          s01 = seq * ( mean(yy$y11) - exp(mlee[1]+e)+ mean(yy$y12) - exp(mlee[1]+e+mlee[4]) + mean(yy$y23)- exp(mlee[1]+e+mlee[5]+mlee[6]) )
...
res11<-do.call(rbind, lapply(c(25,50,150,250), sim.BBA1, dataType="Poisson")) 
write_xlsx(data.frame(res11),path = paste0(Path,"/BBA1_funPo_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))

res12<-do.call(rbind, lapply(c(25,50,150,250), sim.BBA1, dataType="Weibull")) 
write_xlsx(data.frame(res12),path = paste0(Path,"/BBA1_funWei_rho=",cor_par,"_",substr(Sys.Date(),6,10),".xlsx"))
